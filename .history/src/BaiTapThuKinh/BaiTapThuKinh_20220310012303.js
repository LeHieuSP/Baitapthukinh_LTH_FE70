...
export default class BaiTapThuKinh extends Component {
  ...
  renderGlassesList = () => {
    return dataGlasses.map((glassesItem,index)=>{
      return <img className="ml-2 p-2 border border-width-1" style={{width:'100px', cursor:'pointer'}} key={index} src={glassesItem.url} />
    })
  }
          ...
          <div className="container">
            <div className="row mt-5 text-center">
              <div className="col-6">
                ...
                className="position-absolute" src={this.state.glassesCurrent.url} 
                />
                <div style={infoGlasses} className="position-relative">
                  <span style={{color:'#6666FF',fontSize:'18px'}} className="font-weight-bold ">{this.state.glassesCurrent.name}</span> <br/>
                  <span style={{fontSize:'15px',fontWeight:'400'}} className="text-light">{this.state.glassesCurrent.desc}</span>
